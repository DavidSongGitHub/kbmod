...
extern "C" void hostConvolve(float *sourceImg, float *resultImg,
	int width, int height, float *psfKernel,
	int psfSize, int psfDim, int psfRadius, float psfSum)
{
	...
	for (int y=0; y<height; y++)
	{
		...
		{
			const int minX = fmaxf(x-psfRadius, 0);
			const int minY = fmaxf(y-psfRadius, 0);
			const int maxX = fminf(x+psfRadius, width-1);
			const int maxY = fminf(y+psfRadius, height-1);

			// Read kernel
			float sum = 0.0;
			float psfPortion = 0.0;
			float center = sourceImg[y*width+x];
			if (center != NO_DATA) {
				for (int j=minY; j<=maxY; j++)
				{
					// #pragma unroll
					for (int i=minX; i<=maxX; i++)
					{
						float currentPixel = sourceImg[j*width+i];
						if (currentPixel != NO_DATA) {
							float currentPSF = psfKernel[(j-minY)*psfDim+i-minX];
							psfPortion += currentPSF;
							sum += currentPixel * currentPSF;
						}
					}
				...
				resultImg[y*width+x] = (sum*psfSum)/psfPortion;
			} else {
				// Leave masked pixel alone (these could be replaced here with zero)
				resultImg[y*width+x] = NO_DATA; // 0.0
			}
		}
	}
}
...
// Reads a single pixel from an image buffer
float readPixelH(float* img, int x, int y, int width, int height)
{
	return (x<width && y<height) ? img[y*width+x] : NO_DATA;
}

float maxMaskedH(float pixel, float previousMax)
{
	return pixel == NO_DATA ? previousMax : fmaxf(pixel, previousMax);
}

float minMaskedH(float pixel, float previousMin)
{
	return pixel == NO_DATA ? previousMin : fminf(pixel, previousMin);
}
...
extern "C" void hostPool(int sourceWidth, int sourceHeight, float *source,
	int destWidth, int destHeight, float *dest, short mode)
{
	...
	for (int y=0; y<destHeight; y++)
	{
		for (int x=0; x<destWidth; x++)
		{
			...
			if (mode == POOL_MAX) {
				mp = -FLT_MAX;
				pixel = readPixelH(source, 2*x,   2*y,   sourceWidth, sourceHeight);
				mp = maxMaskedH(pixel, mp);
				pixel = readPixelH(source, 2*x+1, 2*y,   sourceWidth, sourceHeight);
				mp = maxMaskedH(pixel, mp);
				pixel = readPixelH(source, 2*x,   2*y+1, sourceWidth, sourceHeight);
				mp = maxMaskedH(pixel, mp);
				pixel = readPixelH(source, 2*x+1, 2*y+1, sourceWidth, sourceHeight);
				mp = maxMaskedH(pixel, mp);
				if (mp == FLT_MIN) mp = NO_DATA;
			} else {
				mp = FLT_MAX;
				pixel = readPixelH(source, 2*x,   2*y,   sourceWidth, sourceHeight);
				mp = minMaskedH(pixel, mp);
				pixel = readPixelH(source, 2*x+1, 2*y,   sourceWidth, sourceHeight);
				mp = minMaskedH(pixel, mp);
				pixel = readPixelH(source, 2*x,   2*y+1, sourceWidth, sourceHeight);
				mp = minMaskedH(pixel, mp);
	...
	for (int y=0; y<height; y++)
	{
		for (int x=0; x<width; x++)
		{
			...
			for (int t=0; t<trajCount; ++t)
			{
				...
				currentT.x = x;
				currentT.y = y;
				currentT.xVel = trajectoriesToSearch[t].xVel;
				currentT.yVel = trajectoriesToSearch[t].yVel;
				currentT.obsCount = 0;
				...
				for (int i=0; i<imageCount; ++i)
				{
					...
					//float cPsi = psiPhiImages[pixel];
					//float cPhi = psiPhiImages[pixel+1];
					float cPsi = interleavedPsiPhi[pixel];
					if (cPsi == NO_DATA) continue;
					float cPhi = interleavedPsiPhi[pixel+1];
					
					currentT.obsCount++;
					psiSum += cPsi;// < NO_DATA/2 /*== NO_DATA* / ? 0.0 : cPsiPhi.x;//min(cPsi,0.3);
					phiSum += cPhi;// < NO_DATA/2 /*== NO_DATA* / ? 0.0 : cPsiPhi.y;
			...
			for (int r=0; r<RESULTS_PER_PIXEL; ++r)
			{
				bestTrajects[ (y*width + x)*RESULTS_PER_PIXEL + r ] = best[r];
			}
		}
	}
}
